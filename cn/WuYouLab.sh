...
UserLicenseAgreement() {
	clear
	echo -e "${gl_kjlan}欢迎使用无忧工坊脚本工具箱${gl_bai}"
	echo "首次使用脚本，请先阅读并同意用户许可协议。"
	echo "用户许可协议: https://blog.wuyoulab.sh/user-license-agreement/"
	echo -e "----------------------"
...
sshkey_panel() {
  ...
  while true; do
	  ...
	  if [[ "$REAL_STATUS" =~ "yes" ]]; then
  	  ...
  	  echo "进阶玩法: https://blog.wuyoulab.sh/ssh-key"
  	  echo "------------------------------------------------"
...
root_use() {
clear
[ "$EUID" -ne 0 ] && echo -e "${gl_huang}提示: ${gl_bai}该功能需要root用户才能运行！" && break_end && WuYouLab
}
...
while true; do
	...
	if [ -z "$sub_choice" ]; then
	  ...
	  echo -e "${gl_kjlan}第三方应用列表"
  	  echo -e "${gl_kjlan}想要让你的应用出现在这里？查看开发者指南: ${gl_huang}https://dev.wuyoulab.sh/${gl_bai}"
	...
	case $sub_choice in
	  ...
	  82|gpt-load)
		...
		docker_rum() {
			...
			read -e -p "设置${docker_name}的登录密钥（sk-开头字母和数字组合）如: sk-159WuYouLabyyds163: " app_passwd
...
linux_Settings() {
	...
	while true; do
	  ...
	  echo -e "${gl_kjlan}101. ${gl_bai}k命令高级用法 ${gl_huang}★${gl_bai}                    ${gl_kjlan}102. ${gl_bai}卸载无忧工坊脚本"
	  echo -e "${gl_kjlan}------------------------"
	  echo -e "${gl_kjlan}0.   ${gl_bai}返回主菜单"
	  echo -e "${gl_kjlan}------------------------${gl_bai}"
	  read -e -p "请输入你的选择: " sub_choice
...
EOF
		  ...
		  21)
			  ...
			  while true; do
				  ...
				  case $host_dns in
					  1)
						  read -e -p "请输入新的解析记录 格式: 110.25.5.33 wuyoulab.sh : " addhost
		  ...
		  61)
			clear
			send_stats "留言板"
			echo "访问无忧工坊官方留言板，您对脚本有任何想法欢迎留言交流！"
			echo "https://board.wuyoulab.sh"
			echo "公共密码: WuYouLab.sh"
			  ;;
...
while true; do
	  ...
	  echo -e "${gl_kjlan}11. ${gl_bai}安装无忧工坊脚本         ${gl_kjlan}12. ${gl_bai}更新系统              ${gl_kjlan}13. ${gl_bai}清理系统"
...
echo -e "${gl_bai}网址: https://wuyoulab.sh/topvps/${gl_bai}"
echo "------------------------"
echo ""
echo -e "域名优惠"
echo "------------------------"
echo -e "${gl_lan}GNAME 8.8刀首年COM域名 6.68刀首年CC域名${gl_bai}"
echo -e "${gl_bai}网址: https://www.gname.com/register?tt=86836&ttcode=WUYOULAB86836&ttbj=sh${gl_bai}"
echo "------------------------"
echo ""
echo -e "无忧工坊周边"
echo "------------------------"
echo -e "${gl_kjlan}B站: ${gl_bai}https://b23.tv/2mqnQyh              ${gl_kjlan}油管: ${gl_bai}https://www.youtube.com/@WuYouLab${gl_bai}"
echo -e "${gl_kjlan}官网: ${gl_bai}https://wuyoulab.sh/              ${gl_kjlan}导航: ${gl_bai}https://dh.wuyoulab.sh/${gl_bai}"
echo -e "${gl_kjlan}博客: ${gl_bai}https://blog.wuyoulab.sh/         ${gl_kjlan}软件中心: ${gl_bai}https://app.wuyoulab.sh/${gl_bai}"
echo "------------------------"
echo -e "${gl_kjlan}脚本官网: ${gl_bai}https://WuYouLab.sh            ${gl_kjlan}GitHub地址: ${gl_bai}${gh_https_url}github.com/wuyou69/WuYouLab${gl_bai}"
echo "------------------------"
echo ""
}
...
while true; do
	...
	echo "更新日志"
	echo "------------------------"
	echo "全部日志: ${gh_proxy}raw.githubusercontent.com/wuyou69/WuYouLab/main/WuYouLab_sh_log.txt"
	echo "------------------------"
