...
root_use() {
clear
[ "$EUID" -ne 0 ] && echo -e "${gl_huang}نکته:${gl_bai}این ویژگی به کاربر root نیاز دارد تا اجرا شود!" && break_end && WuYouLab
}
...
EOF
		  ...
		  102)
			  ...
			  echo "------------------------------------------------"
			  echo "اسکریپت WuYouLab را کاملاً حذف کرده و بر عملکردهای دیگر شما تأثیر نمی گذارد"
			  read -e -p "آیا مطمئناً ادامه خواهید داد؟ (y/n):" choice
...
echo -e "${gl_bai}وب سایت: https://wuyoulab.sh/topvps/${gl_bai}"
echo "------------------------"
echo ""
echo -e "تخفیف نام دامنه"
echo "------------------------"
echo -e "${gl_lan}GNAME 8.8 دلار نام دامنه COM سال اول 6.68 دلار نام دامنه CC سال اول${gl_bai}"
echo -e "${gl_bai}وب سایت: https://www.gname.com/register؟tt=86836&ttcode=WuYouLab86836&ttbj=sh${gl_bai}"
echo "------------------------"
echo ""
echo -e "شیر فناوری اطراف"
echo "------------------------"
echo -e "${gl_kjlan}ایستگاه ب:${gl_bai}https://b23.tv/2mqnQyh              ${gl_kjlan}لوله روغن:${gl_bai}https://www.youtube.com/@WuYouLab${gl_bai}"
echo -e "${gl_kjlan}وب سایت رسمی:${gl_bai}https://wuyoulab.sh/               ${gl_kjlan}ناوبری:${gl_bai}https://dh.wuyoulab.sh/${gl_bai}"
echo -e "${gl_kjlan}وبلاگ:${gl_bai}https://blog.wuyoulab.sh/          ${gl_kjlan}مرکز نرم افزار:${gl_bai}https://app.wuyoulab.sh/${gl_bai}"
echo "------------------------"
echo ""
}
...
while true; do
	...
	echo "------------------------"
	echo "همه گزارش ها:${gh_proxy}raw.githubusercontent.com/wuyou69/WuYouLab/main/WuYouLab_sh_log.txt"
	echo "------------------------"
