...
pal_backup() {
  cd ~
  curl -sS -O https://wuyoulab.sh/pal_backup.sh && chmod +x pal_backup.sh
}
...
echo "[_)(_]| \/\/ (_)[  |(_]"
echo "|                      "
echo -e "\033[96m幻兽帕鲁开服一键脚本工具v1.0.2  by WUYOULAB\033[0m"
...
case $choice in
  ...
  10)
    clear
    tmux kill-session -t my1
    cd ~ && curl -sS -O https://wuyoulab.sh/PalWorldSettings.ini
  ...
  k)
    cd ~
    curl -sS -O https://wuyoulab.sh/WuYouLab.sh && chmod +x WuYouLab.sh && ./WuYouLab.sh
    exit
    ;;

  00)
    cd ~
    curl -sS -O https://wuyoulab.sh/pal_log.sh && chmod +x pal_log.sh && ./pal_log.sh
    rm pal_log.sh
    echo ""
    curl -sS -O https://wuyoulab.sh/palworld.sh && chmod +x palworld.sh
    echo "脚本已更新到最新版本！"
    break_end
    palworld
    ;;
