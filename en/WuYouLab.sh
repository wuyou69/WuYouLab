...
sshkey_panel() {
  ...
  while true; do
	  ...
	  if [[ "$REAL_STATUS" =~ "yes" ]]; then
  	  ...
  	  echo "Advanced gameplay: https://blog.wuyoulab.sh/ssh-key"
  	  echo "------------------------------------------------"
...
root_use() {
clear
[ "$EUID" -ne 0 ] && echo -e "${gl_huang}hint:${gl_bai}This function requires root user to run!" && break_end && WuYouLab
}
...
net.ipv4.tcp_slow_start_after_idle = 0"
	...
	cat > "$CONF" << SYSCTL
# WuYouLab kernel tuning configuration
...
Kernel_optimize() {
	...
	while true; do
	  ...
	  local current_mode=$(grep "^# Mode:" /etc/sysctl.d/99-wuyoulab-optimize.conf 2>/dev/null | sed 's/# mode: //' | awk -F'|' '{print $1}' | xargs)
...
moltbot_menu() {
	...
	show_menu() {
		...
		echo "======================================="
		echo -e "🦞 OPENCLAW Management Tool by WUYOULAB 🦞"
...
while true; do
	...
	if [ -z "$sub_choice" ]; then
	  ...
	  echo -e "${gl_kjlan}Third-party application list"
  	  echo -e "${gl_kjlan}Want your app to appear here? Check out the developer guide:${gl_huang}https://dev.wuyoulab.sh/${gl_bai}"
	...
	case $sub_choice in
	  ...
	  82|gpt-load)
		...
		docker_rum() {
			...
			read -e -p "set up${docker_name}Login key (sk-a combination of letters and numbers starting with) such as: sk-159WuYouLabyyds163:" app_passwd
...
EOF
		  ...
		  21)
			  ...
			  while true; do
				  ...
				  case $host_dns in
					  1)
						  read -e -p "Please enter a new parsing record format: 110.25.5.33 wuyoulab.sh:" addhost
...
echo -e "${gl_bai}Website: https://wuyoulab.sh/topvps/${gl_bai}"
echo "------------------------"
echo ""
...
echo -e "${gl_kjlan}blog:${gl_bai}https://blog.wuyoulab.sh/         ${gl_kjlan}Software Center:${gl_bai}https://app.wuyoulab.sh/${gl_bai}"
echo "------------------------"
echo -e "${gl_kjlan}Script official website:${gl_bai}https://WuYouLab.sh            ${gl_kjlan}GitHub address:${gl_bai}${gh_https_url}github.com/wuyou69/WuYouLab${gl_bai}"
echo "------------------------"
echo ""
}
...
while true; do
	...
	echo "Change log"
	echo "------------------------"
	echo "All logs:${gh_proxy}raw.githubusercontent.com/wuyou69/WuYouLab/main/WuYouLab_sh_log.txt"
	echo "------------------------"
