...
UserLicenseAgreement() {
	clear
	echo -e "${gl_kjlan}歡迎使用无忧工坊腳本工具箱${gl_bai}"
	echo "首次使用腳本，請先閱讀並同意使用者授權協議。"
	echo "使用者授權協議: https://blog.wuyoulab.sh/user-license-agreement/"
	echo -e "----------------------"
...
sshkey_panel() {
  ...
  while true; do
	  ...
	  if [[ "$REAL_STATUS" =~ "yes" ]]; then
  	  ...
  	  echo "進階玩法: https://blog.wuyoulab.sh/ssh-key"
  	  echo "------------------------------------------------"
...
root_use() {
clear
[ "$EUID" -ne 0 ] && echo -e "${gl_huang}提示:${gl_bai}該功能需要root用戶才能運作！" && break_end && WuYouLab
}
...
dd_xitong() {
		...
		dd_xitong_4() {
		  ...
		  while true; do
			...
			echo "重裝系統"
			echo "--------------------------------"
			echo -e "${gl_hong}注意:${gl_bai}重装有风险失联，不放心者慎用。重裝預計花費15分鐘，請提前備份資料。"
...
Kernel_optimize() {
	...
	while true; do
	  ...
	  local current_mode=$(grep "^# 模式:" /etc/sysctl.d/99-wuyoulab-optimize.conf 2>/dev/null | sed 's/# 模式: //' | awk -F'|' '{print $1}' | xargs)
...
moltbot_menu() {
	...
	show_menu() {
		...
		echo "======================================="
		echo -e "🦞 OPENCLAW 管理工具 by WUYOULAB 🦞"
...
while true; do
	...
	if [ -z "$sub_choice" ]; then
	  ...
	  echo -e "${gl_kjlan}第三方應用程式列表"
  	  echo -e "${gl_kjlan}想要讓你的應用程式出現在這裡？查看開發者指南:${gl_huang}https://dev.wuyoulab.sh/${gl_bai}"
	...
	case $sub_choice in
	  ...
	  82|gpt-load)
		...
		docker_rum() {
			...
			read -e -p "設定${docker_name}的登入密鑰（sk-開頭字母和數字組合）如: sk-159WuYouLabyyds163:" app_passwd
...
linux_Settings() {
	...
	while true; do
	  ...
	  echo -e "${gl_kjlan}101. ${gl_bai}k指令進階用法${gl_huang}★${gl_bai}                    ${gl_kjlan}102. ${gl_bai}解除安裝无忧工坊腳本"
	  echo -e "${gl_kjlan}------------------------"
	  echo -e "${gl_kjlan}0.   ${gl_bai}返回主選單"
	  echo -e "${gl_kjlan}------------------------${gl_bai}"
	  read -e -p "請輸入你的選擇:" sub_choice
...
EOF
		  ...
		  21)
			  ...
			  while true; do
				  ...
				  case $host_dns in
					  1)
						  read -e -p "請輸入新的解析記錄 格式: 110.25.5.33 wuyoulab.sh :" addhost
		  ...
		  61)
			clear
			send_stats "留言板"
			echo "造訪无忧工坊官方留言板，您對腳本有任何想法歡迎留言交流！"
			echo "https://board.wuyoulab.sh"
			echo "公共密碼: WuYouLab.sh"
			  ;;
		  ...
		  102)
			  clear
			  send_stats "解除安裝无忧工坊腳本"
			  echo "解除安裝无忧工坊腳本"
			  echo "------------------------------------------------"
			  echo "將徹底卸載WuYouLab腳本，不影響你其他功能"
			  read -e -p "確定繼續嗎？ (Y/N):" choice
...
while true; do
	  ...
	  echo -e "${gl_kjlan}11. ${gl_bai}安裝无忧工坊腳本${gl_kjlan}12. ${gl_bai}更新系統${gl_kjlan}13. ${gl_bai}清理系統"
...
echo -e "${gl_bai}網址: https://wuyoulab.sh/topvps/${gl_bai}"
echo "------------------------"
echo ""
echo -e "網域優惠"
echo "------------------------"
echo -e "${gl_lan}GNAME 8.8刀首年COM域名 6.68刀首年CC域名${gl_bai}"
echo -e "${gl_bai}網址: https://www.gname.com/register?tt=86836&ttcode=WUYOULAB86836&ttbj=sh${gl_bai}"
echo "------------------------"
echo ""
echo -e "无忧工坊週邊"
echo "------------------------"
echo -e "${gl_kjlan}B站:${gl_bai}https://b23.tv/2mqnQyh              ${gl_kjlan}油管:${gl_bai}https://www.youtube.com/@WuYouLab${gl_bai}"
echo -e "${gl_kjlan}官網:${gl_bai}https://wuyoulab.sh/              ${gl_kjlan}導航:${gl_bai}https://dh.wuyoulab.sh/${gl_bai}"
echo -e "${gl_kjlan}部落格:${gl_bai}https://blog.wuyoulab.sh/         ${gl_kjlan}軟體中心:${gl_bai}https://app.wuyoulab.sh/${gl_bai}"
echo "------------------------"
echo -e "${gl_kjlan}腳本官網:${gl_bai}https://WuYouLab.sh            ${gl_kjlan}GitHub地址:${gl_bai}${gh_https_url}github.com/wuyou69/WuYouLab${gl_bai}"
echo "------------------------"
echo ""
}
...
while true; do
	...
	echo "更新日誌"
	echo "------------------------"
	echo "全部日誌:${gh_proxy}raw.githubusercontent.com/wuyou69/WuYouLab/main/WuYouLab_sh_log.txt"
	echo "------------------------"
