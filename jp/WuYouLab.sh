...
sshkey_panel() {
  ...
  while true; do
	  ...
	  if [[ "$REAL_STATUS" =~ "yes" ]]; then
  	  ...
  	  echo "高度なゲームプレイ: https://blog.wuyoulab.sh/ssh-key"
  	  echo "------------------------------------------------"
...
root_use() {
clear
[ "$EUID" -ne 0 ] && echo -e "${gl_huang}ヒント：${gl_bai}この機能を実行するには root ユーザーが必要です。" && break_end && WuYouLab
}
...
Kernel_optimize() {
	...
	while true; do
	  ...
	  local current_mode=$(grep "^# モード:" /etc/sysctl.d/99-wuyoulab-optimize.conf 2>/dev/null | sed 's/# モード: //' | awk -F'|' '{print $1}' | xargs)
...
moltbot_menu() {
	...
	show_menu() {
		...
		echo "======================================="
		echo -e "🦞 WUYOULAB による OPENCLAW 管理ツール 🦞"
...
while true; do
	...
	if [ -z "$sub_choice" ]; then
	  ...
	  echo -e "${gl_kjlan}サードパーティ製アプリケーションのリスト"
  	  echo -e "${gl_kjlan}あなたのアプリをここに表示したいですか?開発者ガイドを確認してください。${gl_huang}https://dev.wuyoulab.sh/${gl_bai}"
	...
	case $sub_choice in
	  ...
	  82|gpt-load)
		...
		docker_rum() {
			...
			read -e -p "設定${docker_name}ログイン キー (sk- で始まる文字と数字の組み合わせ) 例: sk-159WuYouLabyyds163:" app_passwd
...
EOF
		  ...
		  21)
			  ...
			  while true; do
				  ...
				  case $host_dns in
					  1)
						  read -e -p "新しい解析レコード形式を入力してください: 110.25.5.33 wuyoulab.sh:" addhost
...
echo -e "${gl_bai}ウェブサイト：https://wuyoulab.sh/topvps/${gl_bai}"
echo "------------------------"
echo ""
echo -e "ドメイン名の割引"
echo "------------------------"
...
echo -e "${gl_kjlan}ブログ:${gl_bai}https://blog.wuyoulab.sh/         ${gl_kjlan}ソフトウェアセンター:${gl_bai}https://app.wuyoulab.sh/${gl_bai}"
echo "------------------------"
echo -e "${gl_kjlan}スクリプト公式サイト：${gl_bai}https://WuYouLab.sh            ${gl_kjlan}GitHub アドレス:${gl_bai}${gh_https_url}github.com/wuyou69/WuYouLab${gl_bai}"
echo "------------------------"
echo ""
}
...
while true; do
	...
	echo "変更ログ"
	echo "------------------------"
	echo "すべてのログ:${gh_proxy}raw.githubusercontent.com/wuyou69/WuYouLab/main/WuYouLab_sh_log.txt"
	echo "------------------------"
