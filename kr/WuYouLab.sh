...
canshu_v6() {
	if grep -q '^canshu="V6"' /usr/local/bin/k > /dev/null 2>&1; then
		sed -i 's/^canshu="default"/canshu="V6"/' ~/WuYouLab.sh
	elif grep -q '^canshu="V6"' ~/WuYouLab.sh.bak > /dev/null 2>&1; then
		sed -i 's/^canshu="default"/canshu="V6"/' ~/WuYouLab.sh
	fi
}
...
CheckFirstRun_true() {
	if grep -q '^permission_granted="true"' /usr/local/bin/k > /dev/null 2>&1; then
		sed -i 's/^permission_granted="false"/permission_granted="true"/' ~/WuYouLab.sh
	elif grep -q '^permission_granted="true"' ~/WuYouLab.sh.bak > /dev/null 2>&1; then
		sed -i 's/^permission_granted="false"/permission_granted="true"/' ~/WuYouLab.sh
	fi
}
...
send_stats() {
	...
	(
		curl -s -X POST "https://api.wuyoulab.sh/api/log" \
...
if grep -q '^ENABLE_STATS="false"' /usr/local/bin/k > /dev/null 2>&1; then
	sed -i 's/^ENABLE_STATS="true"/ENABLE_STATS="false"/' ~/WuYouLab.sh
elif grep -q '^ENABLE_STATS="false"' ~/WuYouLab.sh.bak > /dev/null 2>&1; then
	sed -i 's/^ENABLE_STATS="true"/ENABLE_STATS="false"/' ~/WuYouLab.sh
fi
...
cp -f ./WuYouLab.sh ~/WuYouLab.sh > /dev/null 2>&1
cp -f ~/WuYouLab.sh /usr/local/bin/k > /dev/null 2>&1
...
UserLicenseAgreement() {
	clear
	echo -e "${gl_kjlan}WuYouLab 스크립트 도구 상자에 오신 것을 환영합니다.${gl_bai}"
	echo "스크립트를 처음 사용하는 경우 사용자 라이센스 계약을 읽고 동의하십시오."
	echo "사용자 라이센스 계약: https://blog.wuyoulab.sh/user-license-agreement/"
	echo -e "----------------------"
...
install_ldnmp_conf() {
  ...
  wget -O /home/web/conf.d/default.conf ${gh_proxy}raw.githubusercontent.com/wuyou69/nginx/main/default10.conf

  default_server_ssl

  # docker-compose.yml 파일을 다운로드하여 교체하세요.
  wget -O /home/web/docker-compose.yml ${gh_proxy}raw.githubusercontent.com/wuyou69/docker/main/LNMP-docker-compose-10.yml
  dbrootpasswd=$(openssl rand -base64 16) ; dbuse=$(openssl rand -hex 4) ; dbusepasswd=$(openssl rand -base64 8)

  # docker-compose.yml 파일에서 교체
  sed -i "s#webroot#$dbrootpasswd#g" /home/web/docker-compose.yml
  sed -i "s#WuYouLabYYDS#$dbusepasswd#g" /home/web/docker-compose.yml
...
install_ldnmp() {
	  ...
	  # MySQL 튜닝
	  wget -O /home/custom_mysql_config.cnf ${gh_proxy}raw.githubusercontent.com/wuyou69/WuYouLab/main/custom_mysql_config-1.cnf
...
install_certbot() {
	...
	cd ~
	curl -sS -O ${gh_proxy}raw.githubusercontent.com/wuyou69/WuYouLab/main/auto_cert_renewal.sh
...
phpmyadmin_upgrade() {
  ...
  docker rm -f $ldnmp_pods > /dev/null 2>&1
  docker images --filter=reference="$ldnmp_pods*" -q | xargs docker rmi > /dev/null 2>&1
  curl -sS -O ${gh_proxy}raw.githubusercontent.com/wuyou69/docker/refs/heads/main/docker-compose.phpmyadmin.yml
		...
		check_cf_mode
			  ...
			  case $sub_choice in
				  1)
					  ...
					  curl -sS -O ${gh_proxy}raw.githubusercontent.com/wuyou69/WuYouLab/main/fail2ban-nginx-cc.conf
					  ...
					  curl -sS -O ${gh_proxy}raw.githubusercontent.com/wuyou69/config/main/fail2ban/nginx-docker-cc.conf
				  ...
				  21)
					  ...
					  wget -O /home/web/conf.d/default.conf ${gh_proxy}raw.githubusercontent.com/wuyou69/nginx/main/default11.conf
					  docker exec nginx nginx -s reload
					  ...
					  curl -sS -O ${gh_proxy}raw.githubusercontent.com/wuyou69/config/main/fail2ban/cloudflare-docker.conf

					  sed -i "s/WuYouLab@outlook.com/$cfuser/g" /etc/fail2ban/action.d/cloudflare-docker.conf
				  ...
				  22)
					  ...
					  install jq bc
					  check_crontab_installed
					  curl -sS -O ${gh_proxy}raw.githubusercontent.com/wuyou69/WuYouLab/main/CF-Under-Attack.sh
...
web_optimization() {
		  while true; do
			  ...
			  case $sub_choice in
				  ...
				  # PHP 튜닝
				  wget -O /home/optimized_php.ini ${gh_proxy}raw.githubusercontent.com/wuyou69/WuYouLab/main/optimized_php.ini
				  docker cp /home/optimized_php.ini php:/usr/local/etc/php/conf.d/optimized_php.ini
				  docker cp /home/optimized_php.ini php74:/usr/local/etc/php/conf.d/optimized_php.ini
				  rm -rf /home/optimized_php.ini

				  # PHP 튜닝
				  wget -O /home/www.conf ${gh_proxy}raw.githubusercontent.com/wuyou69/WuYouLab/main/www-1.conf
				  ...
				  # PHP 튜닝
				  wget -O /home/www.conf ${gh_proxy}raw.githubusercontent.com/wuyou69/WuYouLab/main/www.conf
				  docker cp /home/www.conf php:/usr/local/etc/php-fpm.d/www.conf
				  docker cp /home/www.conf php74:/usr/local/etc/php-fpm.d/www.conf
				  rm -rf /home/www.conf
				  ...
				  # MySQL 튜닝
				  wget -O /home/custom_mysql_config.cnf ${gh_proxy}raw.githubusercontent.com/wuyou69/WuYouLab/main/custom_mysql_config.cnf
				  docker cp /home/custom_mysql_config.cnf mysql:/etc/mysql/conf.d/
				  rm -rf /home/custom_mysql_config.cnf
...
if [ ! -f "$PROMETHEUS_DIR/prometheus.yml" ]; then
	curl -o "$PROMETHEUS_DIR/prometheus.yml" ${gh_proxy}raw.githubusercontent.com/wuyou69/config/refs/heads/main/prometheus/prometheus.yml
fi
...
f2b_install_sshd() {
	...
	if command -v dnf &>/dev/null; then
		cd /etc/fail2ban/jail.d/
		curl -sS -O ${gh_proxy}raw.githubusercontent.com/wuyou69/config/main/fail2ban/centos-ssh.conf
	fi
...
ldnmp_wp() {
  ...
  wget -O /home/web/conf.d/map.conf ${gh_proxy}raw.githubusercontent.com/wuyou69/nginx/main/map.conf
  wget -O /home/web/conf.d/$yuming.conf ${gh_proxy}raw.githubusercontent.com/wuyou69/nginx/main/wordpress.com.conf
  ...
  mkdir $yuming
  cd $yuming
  wget -O latest.zip ${gh_proxy}github.com/wuyou69/Website_source_code/raw/refs/heads/main/wp-latest.zip
  unzip latest.zip
  rm latest.zip
...
ldnmp_Proxy() {
	...
	wget -O /home/web/conf.d/map.conf ${gh_proxy}raw.githubusercontent.com/wuyou69/nginx/main/map.conf
	wget -O /home/web/conf.d/$yuming.conf ${gh_proxy}raw.githubusercontent.com/wuyou69/nginx/main/reverse-proxy-backend.conf
...
ldnmp_Proxy_backend_stream() {
	...
	wget -O /home/web/stream.d/$proxy_name.conf ${gh_proxy}raw.githubusercontent.com/wuyou69/nginx/main/reverse-proxy-backend-stream.conf
...
local CONF="/etc/sysctl.d/99-wuyoulab-bbr.conf"
...
sshkey_panel() {
  ...
  while true; do
	  ...
	  if [[ "$REAL_STATUS" =~ "yes" ]]; then
  	  ...
  	  echo "고급 게임플레이: https://blog.wuyoulab.sh/ssh-key"
  	  echo "------------------------------------------------"
...
root_use() {
clear
[ "$EUID" -ne 0 ] && echo -e "${gl_huang}힌트:${gl_bai}이 기능을 실행하려면 루트 사용자가 필요합니다!" && break_end && WuYouLab
}
...
bbrv3() {
		  ...
		  xanmod_add_repo() {
				...
				local fallback_key_url="${gh_proxy}raw.githubusercontent.com/wuyou69/WuYouLab/main/archive.key"
				local os_codename=""
...
_kernel_optimize_core() {
	local mode_name="$1"
	local scene="${2:-high}"
	local CONF="/etc/sysctl.d/99-wuyoulab-optimize.conf"
...
SYSCTL
	...
	# ── 파일 설명자 제한 사항 ──
	if ! grep -q "# wuyoulab-optimize" /etc/security/limits.conf 2>/dev/null; then
...
restore_defaults() {
	...
	# 파일 설명자 구성 정리
	if grep -q "# wuyoulab-optimize" /etc/security/limits.conf 2>/dev/null; then
		sed -i '/# wuyoulab-optimize/,+4d' /etc/security/limits.conf
	fi
...
Kernel_optimize() {
	...
	while true; do
	  ...
	  local current_mode=$(grep "^# 모드:" /etc/sysctl.d/99-wuyoulab-optimize.conf 2>/dev/null | sed 's/# 모드: //' | awk -F'|' '{print $1}' | xargs)
	  ...
	  case $sub_choice in
		  ...
		  6)
			  ...
			  clear
			  restore_defaults
			  curl -sS ${gh_proxy}raw.githubusercontent.com/wuyou69/WuYouLab/refs/heads/main/network-optimize.sh -o /tmp/network-optimize.sh && source /tmp/network-optimize.sh && restore_network_defaults
			  send_stats "기본 설정 복원"
			  ;;
		  ...
		  7)
			  cd ~
			  clear
			  curl -sS ${gh_proxy}raw.githubusercontent.com/wuyou69/WuYouLab/refs/heads/main/network-optimize.sh | bash
			  send_stats "커널 자동 튜닝"
			  ;;
...
linux_ldnmp() {
  while true; do
	...
	case $sub_choice in
	  ...
	  wget -O /home/web/conf.d/map.conf ${gh_proxy}raw.githubusercontent.com/wuyou69/nginx/main/map.conf
	  wget -O /home/web/conf.d/$yuming.conf ${gh_proxy}raw.githubusercontent.com/wuyou69/nginx/main/discuz.com.conf
	  sed -i "s/yuming.com/$yuming/g" /home/web/conf.d/$yuming.conf
	  ...
	  mkdir $yuming
	  cd $yuming
	  wget -O latest.zip ${gh_proxy}github.com/wuyou69/Website_source_code/raw/main/Discuz_X3.5_SC_UTF8_20250901.zip
	  unzip latest.zip
	  rm latest.zip
	  ...
	  wget -O /home/web/conf.d/map.conf ${gh_proxy}raw.githubusercontent.com/wuyou69/nginx/main/map.conf
	  wget -O /home/web/conf.d/$yuming.conf ${gh_proxy}raw.githubusercontent.com/wuyou69/nginx/main/kdy.com.conf
	  sed -i "s/yuming.com/$yuming/g" /home/web/conf.d/$yuming.conf
	  ...
	  wget -O /home/web/conf.d/map.conf ${gh_proxy}raw.githubusercontent.com/wuyou69/nginx/main/map.conf
	  wget -O /home/web/conf.d/$yuming.conf ${gh_proxy}raw.githubusercontent.com/wuyou69/nginx/main/maccms.com.conf
	  sed -i "s/yuming.com/$yuming/g" /home/web/conf.d/$yuming.conf
	  ...
	  cd /home/web/html/$yuming/template/ && wget ${gh_proxy}github.com/wuyou69/Website_source_code/raw/main/DYXS2.zip && unzip DYXS2.zip && rm /home/web/html/$yuming/template/DYXS2.zip
	  cp /home/web/html/$yuming/template/DYXS2/asset/admin/Dyxs2.php /home/web/html/$yuming/application/admin/controller
	  cp /home/web/html/$yuming/template/DYXS2/asset/admin/dycms.html /home/web/html/$yuming/application/admin/view/system
	  mv /home/web/html/$yuming/admin.php /home/web/html/$yuming/vip.php && wget -O /home/web/html/$yuming/application/extra/maccms.php ${gh_proxy}raw.githubusercontent.com/wuyou69/Website_source_code/main/maccms.php
	  ...
	  wget -O /home/web/conf.d/map.conf ${gh_proxy}raw.githubusercontent.com/wuyou69/nginx/main/map.conf
	  wget -O /home/web/conf.d/$yuming.conf ${gh_proxy}raw.githubusercontent.com/wuyou69/nginx/main/dujiaoka.com.conf
	  sed -i "s/yuming.com/$yuming/g" /home/web/conf.d/$yuming.conf
	  ...
	  wget -O /home/web/conf.d/map.conf ${gh_proxy}raw.githubusercontent.com/wuyou69/nginx/main/map.conf
	  wget -O /home/web/conf.d/$yuming.conf ${gh_proxy}raw.githubusercontent.com/wuyou69/nginx/main/flarum.com.conf
	  sed -i "s/yuming.com/$yuming/g" /home/web/conf.d/$yuming.conf
	  ...
	  wget -O /home/web/conf.d/map.conf ${gh_proxy}raw.githubusercontent.com/wuyou69/nginx/main/map.conf
	  wget -O /home/web/conf.d/$yuming.conf ${gh_proxy}raw.githubusercontent.com/wuyou69/nginx/main/typecho.com.conf
	  sed -i "s/yuming.com/$yuming/g" /home/web/conf.d/$yuming.conf
	  ...
	  wget -O /home/web/conf.d/map.conf ${gh_proxy}raw.githubusercontent.com/wuyou69/nginx/main/map.conf
	  wget -O /home/web/conf.d/$yuming.conf ${gh_proxy}raw.githubusercontent.com/wuyou69/nginx/refs/heads/main/index_php.conf
	  ...
	  wget -O /home/web/conf.d/map.conf ${gh_proxy}raw.githubusercontent.com/wuyou69/nginx/main/map.conf
	  wget -O /home/web/conf.d/$yuming.conf ${gh_proxy}raw.githubusercontent.com/wuyou69/nginx/main/index_php.conf
	  sed -i "s/yuming.com/$yuming/g" /home/web/conf.d/$yuming.conf
	  ...
	  wget -O /home/web/conf.d/$yuming.conf ${gh_proxy}raw.githubusercontent.com/wuyou69/nginx/main/rewrite.conf
	  sed -i "s/yuming.com/$yuming/g" /home/web/conf.d/$yuming.conf
	  ...
	  wget -O /home/web/conf.d/$yuming.conf ${gh_proxy}raw.githubusercontent.com/wuyou69/nginx/main/reverse-proxy-domain.conf
	  sed -i "s/yuming.com/$yuming/g" /home/web/conf.d/$yuming.conf
	  ...
	  wget -O /home/web/conf.d/$yuming.conf ${gh_proxy}raw.githubusercontent.com/wuyou69/nginx/main/html.conf
	  sed -i "s/yuming.com/$yuming/g" /home/web/conf.d/$yuming.conf
	  ...
	  wget ${gh_proxy}github.com/wuyou69/Website_source_code/raw/refs/heads/main/ai_prompt_generator.zip
	  unzip $(ls -t *.zip | head -n 1)
	  rm -f $(ls -t *.zip | head -n 1)
	...
	33)
	  ...
	  cd ~
	  wget -O ${useip}_beifen.sh ${gh_proxy}raw.githubusercontent.com/wuyou69/WuYouLab/main/beifen.sh > /dev/null 2>&1
...
moltbot_menu() {
	...
	show_menu() {
		...
		echo "======================================="
		echo -e "🦞 WUYOULAB의 OPENCLAW 관리 도구 🦞"
...
EOF
	...
	add-openclaw-provider-interactive() {
		...
		# 2. Base URL
		read -erp "기본 URL을 입력하십시오(예: https://api.xxx.com/v1):" base_url
		while [[ -z "$base_url" ]]; do
...
')
	...
	openclaw_memory_select_sources() {
		...
		if [ "$OPENCLAW_MEMORY_USE_MIRROR" = "true" ]; then
			...
			OPENCLAW_MEMORY_GH_PROXY="https://gh-proxy.com/"
		else
			if [ "$hf_ok" = "ok" ]; then
				OPENCLAW_MEMORY_HF_BASE="https://huggingface.co"
			elif [ "$mirror_ok" = "ok" ]; then
		...
		while true; do
			...
			case "$memory_choice" in
				...
				if [ "$confirm_step2" = "force" ]; then
					...
					read -e -p "세 번째 확인: 재구축을 입력하여 안전한 재구축을 수행합니다. Enter를 눌러 수직력을 계속 사용합니다." confirm_step3
...
if [ ! -d apps/.git ]; then
	timeout 10s git clone ${gh_proxy}github.com/wuyou69/apps.git
else
	cd apps
	# git pull origin main > /dev/null 2>&1
	timeout 10s git pull ${gh_proxy}github.com/wuyou69/apps.git main > /dev/null 2>&1
fi
...
while true; do
	...
	if [ -z "$sub_choice" ]; then
	  ...
	  echo -e "${gl_kjlan}타사 애플리케이션 목록"
  	  echo -e "${gl_kjlan}귀하의 앱이 여기에 표시되기를 원하십니까? 개발자 가이드를 확인하세요:${gl_huang}https://dev.wuyoulab.sh/${gl_bai}"
	...
	case $sub_choice in
	  ...
	  13|cloudreve)
		...
		docker_app_install() {
			...
			curl -o /home/docker/cloud/docker-compose.yml ${gh_proxy}raw.githubusercontent.com/wuyou69/docker/main/cloudreve-docker-compose.yml
	  ...
	  74|moontv)
		...
		docker_app_install() {
			...
			curl -o /home/docker/moontv/docker-compose.yml ${gh_proxy}raw.githubusercontent.com/wuyou69/docker/main/moontv-docker-compose.yml
	  ...
	  82|gpt-load)
		...
		docker_rum() {
			...
			read -e -p "설정${docker_name}로그인 키(sk-다음과 같이 시작하는 문자와 숫자의 조합): sk-159WuYouLabyyds163:" app_passwd
	  ...
	  91|gitea)
		...
		docker_app_install() {
			...
			curl -o /home/docker/gitea/docker-compose.yml ${gh_proxy}raw.githubusercontent.com/wuyou69/docker/main/gitea-docker-compose.yml
	...
	94|gopeed)
	  ...
	  96|2fauth)
		...
		docker_app_install() {
			...
			curl -o /home/docker/2fauth/docker-compose.yml ${gh_proxy}raw.githubusercontent.com/wuyou69/docker/main/2fauth-docker-compose.yml
	...
	98|wgc)
	  ...
	  99|dsm)
		...
		docker_app_install() {
			...
			curl -o /home/docker/dsm/docker-compose.yml ${gh_proxy}raw.githubusercontent.com/wuyou69/docker/main/dsm-docker-compose.yml
...
discourse,yunsou,ahhhhfs,nsgame,gying" \
	  ...
	  115|hermes)
	  	  bash <(curl -sL ${gh_proxy}raw.githubusercontent.com/wuyou69/WuYouLab/main/hermes_manager.sh)
		  ;;
...
linux_Settings() {
	...
	while true; do
	  ...
	  echo -e "${gl_kjlan}101. ${gl_bai}k 명령의 고급 사용법${gl_huang}★${gl_bai}                    ${gl_kjlan}102. ${gl_bai}WuYouLab 스크립트 제거"
	  echo -e "${gl_kjlan}------------------------"
	  echo -e "${gl_kjlan}0.   ${gl_bai}메인 메뉴로 돌아가기"
	  echo -e "${gl_kjlan}------------------------${gl_bai}"
	  read -e -p "선택사항을 입력하세요:" sub_choice
...
EOF
		  ...
		  21)
			  ...
			  while true; do
				  ...
				  case $host_dns in
					  1)
						  read -e -p "새로운 구문 분석 기록 형식을 입력하세요: 110.25.5.33 wuyoulab.sh:" addhost
			...
			while true; do
				...
				case "$Limiting" in
				  1)
					...
					cd ~
					curl -Ss -o ~/Limiting_Shut_down.sh ${gh_proxy}raw.githubusercontent.com/wuyou69/WuYouLab/main/Limiting_Shut_down1.sh
			  ...
			  case "$choice" in
				[Yy])
				  ...
				  if [ -f ~/TG-check-notify.sh ]; then
					  ...
					  curl -sS -O ${gh_proxy}raw.githubusercontent.com/wuyou69/WuYouLab/main/TG-check-notify.sh
					  chmod +x ~/TG-check-notify.sh
					  nano ~/TG-check-notify.sh
				  fi
				  ...
				  curl -sS -O ${gh_proxy}raw.githubusercontent.com/wuyou69/WuYouLab/main/TG-SSH-check-notify.sh > /dev/null 2>&1
		  ...
		  26)
			  ...
			  cd ~
			  curl -sS -O ${gh_proxy}raw.githubusercontent.com/wuyou69/WuYouLab/main/upgrade_openssh9.8p1.sh
		  ...
		  61)
			clear
			send_stats "메시지 보드"
			echo "WuYouLab 공식 게시판을 방문해 보세요. 스크립트에 대한 아이디어가 있으시면 교환 메시지를 남겨주세요!"
			echo "https://board.wuyoulab.sh"
			echo "공개 비밀번호: WuYouLab.sh"
			  ;;
			...
			while true; do
			  ...
			  case $sub_choice in
				  1)
					  ...
					  sed -i 's/^ENABLE_STATS="false"/ENABLE_STATS="true"/' ~/WuYouLab.sh
					  echo "수집이 시작되었습니다"
		  ...
		  102)
			  clear
			  send_stats "WuYouLab 스크립트 제거"
			  echo "WuYouLab 스크립트 제거"
			  echo "------------------------------------------------"
			  echo "WuYouLab 스크립트는 다른 기능에 영향을 주지 않고 완전히 제거됩니다."
			  read -e -p "계속하시겠습니까? (예/아니요):" choice

			  case "$choice" in
				[Yy])
				  clear
				  (crontab -l | grep -v "WuYouLab.sh") | crontab -
...
cluster_python3() {
	...
	cd ~/cluster/
	curl -sS -O ${gh_proxy}raw.githubusercontent.com/wuyou69/python-for-vps/main/cluster/$py_task
...
while true; do
	  ...
	  echo -e "${gl_kjlan}11. ${gl_bai}WuYouLab 스크립트 설치${gl_kjlan}12. ${gl_bai}시스템 업데이트${gl_kjlan}13. ${gl_bai}시스템 청소"
	  ...
	  case $sub_choice in
		  ...
		  11)
			  local py_task="install_WuYouLab.py"
			  cluster_python3
			  ;;
		  12)
...
echo -e "${gl_bai}홈페이지: https://wuyoulab.sh/topvps/${gl_bai}"
echo "------------------------"
echo ""
echo -e "도메인 이름 할인"
echo "------------------------"
echo -e "${gl_lan}GNAME $8.8 첫해 COM 도메인 이름 $6.68 첫해 CC 도메인 이름${gl_bai}"
echo -e "${gl_bai}웹사이트: https://www.gname.com/register?tt=86836&ttcode=WUYOULAB86836&ttbj=sh${gl_bai}"
echo "------------------------"
echo ""
echo -e "WuYouLab 주변기기"
echo "------------------------"
echo -e "${gl_kjlan}스테이션 B:${gl_bai}https://b23.tv/2mqnQyh              ${gl_kjlan}오일 파이프:${gl_bai}https://www.youtube.com/@WuYouLab${gl_bai}"
echo -e "${gl_kjlan}공식 웹사이트:${gl_bai}https://wuyoulab.sh/              ${gl_kjlan}항해:${gl_bai}https://dh.wuyoulab.sh/${gl_bai}"
echo -e "${gl_kjlan}블로그:${gl_bai}https://blog.wuyoulab.sh/         ${gl_kjlan}소프트웨어 센터:${gl_bai}https://app.wuyoulab.sh/${gl_bai}"
echo "------------------------"
echo -e "${gl_kjlan}스크립트 공식 웹사이트:${gl_bai}https://WuYouLab.sh            ${gl_kjlan}GitHub 주소:${gl_bai}${gh_https_url}github.com/wuyou69/WuYouLab${gl_bai}"
echo "------------------------"
echo ""
}
	...
	while true; do
	  ...
	  case $sub_choice in
		  ...
		  1) send_stats "Eudemons Parlu 서버 오픈 스크립트" ; cd ~
			 curl -sS -O ${gh_proxy}raw.githubusercontent.com/wuyou69/WuYouLab/main/palworld.sh ; chmod +x palworld.sh ; ./palworld.sh
			 exit
			 ;;
		  2) send_stats "Minecraft 서버 열기 스크립트" ; cd ~
			 curl -sS -O ${gh_proxy}raw.githubusercontent.com/wuyou69/WuYouLab/main/mc.sh ; chmod +x mc.sh ; ./mc.sh
			 exit
			 ;;
...
while true; do
	...
	echo "변경 로그"
	echo "------------------------"
	echo "모든 로그:${gh_proxy}raw.githubusercontent.com/wuyou69/WuYouLab/main/WuYouLab_sh_log.txt"
	echo "------------------------"

	curl -s --max-time 15 ${gh_proxy}raw.githubusercontent.com/wuyou69/WuYouLab/main/WuYouLab_sh_log.txt | tail -n 30
	# 전체 스크립트를 다운로드하지 않으려면 처음 5줄만 다운로드하여 버전 번호를 확인하세요.
	local sh_v_new=$(curl -s --max-time 15 -r 0-200 ${gh_proxy}raw.githubusercontent.com/wuyou69/WuYouLab/main/WuYouLab.sh | grep -o 'sh_v="[0-9.]*"' | head -1 | cut -d '"' -f 2)
	...
	case "$choice" in
		1)
			...
			local download_url
			if [ "$country" = "CN" ]; then
				download_url="${gh_proxy}raw.githubusercontent.com/wuyou69/WuYouLab/main/cn/WuYouLab.sh"
			else
				download_url="${gh_proxy}raw.githubusercontent.com/wuyou69/WuYouLab/main/WuYouLab.sh"
			fi

			# 현재 스크립트 백업
			cp -f ~/WuYouLab.sh ~/WuYouLab.sh.bak 2>/dev/null

			# 임시 파일로 다운로드 후 확인 후 교체
			local tmp_file=$(mktemp ~/WuYouLab_tmp.XXXXXX)
			if curl -sS --max-time 60 --fail -o "$tmp_file" "$download_url" && \
			   [ -s "$tmp_file" ] && \
			   head -1 "$tmp_file" | grep -q '^#!/bin/bash'; then
				chmod +x "$tmp_file"
				mv -f "$tmp_file" ~/WuYouLab.sh
				canshu_v6
				CheckFirstRun_true
				yinsiyuanquan2
				cp -f ~/WuYouLab.sh /usr/local/bin/k > /dev/null 2>&1
				...
				rm -f "$tmp_file"
				# 백업 복원
				if [ -f ~/WuYouLab.sh.bak ]; then
					mv -f ~/WuYouLab.sh.bak ~/WuYouLab.sh
				fi
		...
		2)
			...
			if [ "$country" = "CN" ]; then
				cron_proxy="https://gh-proxy.com/"
				cron_sed_cmd="sed -i 's/canshu=\"default\"/canshu=\"CN\"/g' ~/WuYouLab.sh"
			elif [ -n "$ipv6_address" ]; then
				cron_proxy="https://gh-proxy.com/"
				cron_sed_cmd="sed -i 's/canshu=\"default\"/canshu=\"V6\"/g' ~/WuYouLab.sh"
			else
				cron_proxy="https://"
				cron_sed_cmd=""
			fi
			...
			SH_Update_task="cd ~ && tmp=\$(mktemp ~/WuYouLab_tmp.XXXXXX) && curl -sS --max-time 60 --fail -o \"\$tmp\" ${cron_proxy}raw.githubusercontent.com/wuyou69/WuYouLab/main/WuYouLab.sh && [ -s \"\$tmp\" ] && head -1 \"\$tmp\" | grep -q '^#!/bin/bash' && cp -f ~/WuYouLab.sh ~/WuYouLab.sh.bak 2>/dev/null && chmod +x \"\$tmp\" && mv -f \"\$tmp\" ~/WuYouLab.sh"
			# 추가 설정 복구
			if [ -n "$cron_sed_cmd" ]; then
				SH_Update_task="$SH_Update_task && $cron_sed_cmd"
			fi
			# 이전 스크립트에서Permission_granted 및 ENABLE_STATS 설정을 복원합니다.
			SH_Update_task="$SH_Update_task && grep -q 'permission_granted=\"true\"' ~/WuYouLab.sh.bak 2>/dev/null && sed -i 's/permission_granted=\"false\"/permission_granted=\"true\"/' ~/WuYouLab.sh; grep -q 'ENABLE_STATS=\"false\"' ~/WuYouLab.sh.bak 2>/dev/null && sed -i 's/ENABLE_STATS=\"true\"/ENABLE_STATS=\"false\"/' ~/WuYouLab.sh"
			# /usr/local/bin/k 및 /usr/bin/k에 배포
			SH_Update_task="$SH_Update_task; cp -f ~/WuYouLab.sh /usr/local/bin/k 2>/dev/null; ln -sf /usr/local/bin/k /usr/bin/k 2>/dev/null"
			# 다운로드 실패 시 임시 파일 정리
...
WUYOULAB_ASCII
echo -e "WuYouLab 스크립트 도구 상자 v$sh_v"
