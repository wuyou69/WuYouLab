...
if [ "$country" = "CN" ]; then
	cd ~
	curl -sS -O ${gh_proxy}raw.githubusercontent.com/wuyou69/docker/main/install && chmod +x install
...
install_ldnmp_conf() {
  ...
  wget -O /home/web/redis/valkey.conf ${gh_proxy}raw.githubusercontent.com/wuyou69/WuYouLab/main/valkey.conf
...
reverse_proxy() {
	  ip_address
	  wget -O /home/web/conf.d/$yuming.conf ${gh_proxy}raw.githubusercontent.com/wuyou69/nginx/main/reverse-proxy.conf
...
phpmyadmin_upgrade() {
  ...
  docker rm -f $ldnmp_pods > /dev/null 2>&1
  docker images --filter=reference="$ldnmp_pods*" -q | xargs docker rmi > /dev/null 2>&1
  curl -sS -O https://raw.githubusercontent.com/wuyou69/docker/refs/heads/main/docker-compose.phpmyadmin.yml
...
f2b_install_sshd() {
	...
	if grep -q 'Alpine' /etc/issue; then
		...
		curl -sS -O ${gh_proxy}raw.githubusercontent.com/wuyou69/config/main/fail2ban/alpine-sshd.conf
		curl -sS -O ${gh_proxy}raw.githubusercontent.com/wuyou69/config/main/fail2ban/alpine-sshd-ddos.conf
		cd /path/to/fail2ban/config/fail2ban/jail.d/
		curl -sS -O ${gh_proxy}raw.githubusercontent.com/wuyou69/config/main/fail2ban/alpine-ssh.conf
	elif command -v dnf &>/dev/null; then
		cd /path/to/fail2ban/config/fail2ban/jail.d/
		...
		cd /path/to/fail2ban/config/fail2ban/jail.d/
		curl -sS -O ${gh_proxy}raw.githubusercontent.com/wuyou69/config/main/fail2ban/linux-ssh.conf
...
root_use() {
clear
[ "$EUID" -ne 0 ] && echo -e "${gl_huang}намекать:${gl_bai}Эта функция требует, чтобы root -пользователь запустил!" && break_end && WuYouLab
}
			...
			while true; do
				  ...
				  case $sub_choice in
					  1)
						...
						wget -qO - ${gh_proxy}raw.githubusercontent.com/wuyou69/WuYouLab/main/archive.key | gpg --dearmor -o /usr/share/keyrings/xanmod-archive-keyring.gpg --yes

						# Шаг 3: Добавьте репозиторий
						echo 'deb [signed-by=/usr/share/keyrings/xanmod-archive-keyring.gpg] http://deb.xanmod.org releases main' | tee /etc/apt/sources.list.d/xanmod-release.list

						# version=$(wget -q https://dl.xanmod.org/check_x86-64_psabi.sh && chmod +x check_x86-64_psabi.sh && ./check_x86-64_psabi.sh | grep -oP 'x86-64-v\K\d+|x86-64-v\d+')
						local version=$(wget -q ${gh_proxy}raw.githubusercontent.com/wuyou69/WuYouLab/main/check_x86-64_psabi.sh && chmod +x check_x86-64_psabi.sh && ./check_x86-64_psabi.sh | grep -oP 'x86-64-v\K\d+|x86-64-v\d+')
...
linux_ldnmp() {
  while true; do
	...
	case $sub_choice in
	  ...
	  mkdir $yuming
	  cd $yuming
	  wget -O latest.zip ${gh_proxy}github.com/wuyou69/Website_source_code/raw/main/Discuz_X3.5_SC_UTF8_20240520.zip
	  unzip latest.zip
	  rm latest.zip
			...
			rm -rf /etc/fail2ban
			  ...
			  case $sub_choice in
				  ...
				  21)
					  ...
					  sed -i "s/WuYouLab@outlook.com/$cfuser/g" /path/to/fail2ban/config/fail2ban/action.d/cloudflare-docker.conf
...
EOF
		  ...
		  21)
			  ...
			  while true; do
				  ...
				  case $host_dns in
					  1)
						  read -e -p "Пожалуйста, введите новый формат записи анализа: 110.25.5.33 wuyoulab.sh:" addhost
...
echo -e "${gl_bai}Веб -сайт: https://wuyoulab.sh/topvps/${gl_bai}"
echo "------------------------"
echo ""
...
echo -e "${gl_bai}Веб -сайт: https://www.gname.com/register?tt=86836&ttcode=WuYouLab86836&ttbj=sh${gl_bai}"
echo "------------------------"
echo ""
echo -e "Технологический лев окружает"
echo "------------------------"
echo -e "${gl_kjlan}B Станция:${gl_bai}https://b23.tv/2mqnQyh              ${gl_kjlan}Масляная труба:${gl_bai}https://www.youtube.com/@WuYouLab${gl_bai}"
echo -e "${gl_kjlan}Официальный веб -сайт:${gl_bai}https://wuyoulab.sh/               ${gl_kjlan}Навигация:${gl_bai}https://dh.wuyoulab.sh/${gl_bai}"
echo -e "${gl_kjlan}Блог:${gl_bai}https://blog.wuyoulab.sh/          ${gl_kjlan}Центр программного обеспечения:${gl_bai}https://app.wuyoulab.sh/${gl_bai}"
echo "------------------------"
echo ""
}
...
while true; do
	...
	echo "------------------------"
	echo "Все журналы:${gh_proxy}raw.githubusercontent.com/wuyou69/WuYouLab/main/WuYouLab_sh_log.txt"
	echo "------------------------"

	curl -s ${gh_proxy}raw.githubusercontent.com/wuyou69/WuYouLab/main/WuYouLab_sh_log.txt | tail -n 30
	local sh_v_new=$(curl -s ${gh_proxy}raw.githubusercontent.com/wuyou69/WuYouLab/main/WuYouLab.sh | grep -o 'sh_v="[0-9.]*"' | cut -d '"' -f 2)
	...
	case "$choice" in
		1)
			...
			local country=$(curl -s ipinfo.io/country)
			if [ "$country" = "CN" ]; then
				curl -sS -O ${gh_proxy}raw.githubusercontent.com/wuyou69/WuYouLab/main/cn/WuYouLab.sh && chmod +x WuYouLab.sh
			else
				curl -sS -O ${gh_proxy}raw.githubusercontent.com/wuyou69/WuYouLab/main/WuYouLab.sh && chmod +x WuYouLab.sh
			fi
			canshu_v6
			CheckFirstRun_true
			yinsiyuanquan2
		...
		2)
			...
			if [ "$country" = "CN" ]; then
				SH_Update_task="curl -sS -O https://gh-proxy.com/raw.githubusercontent.com/wuyou69/WuYouLab/main/WuYouLab.sh && chmod +x WuYouLab.sh && sed -i 's/canshu=\"default\"/canshu=\"CN\"/g' ./WuYouLab.sh"
			elif [ -n "$ipv6_address" ]; then
				SH_Update_task="curl -sS -O https://gh-proxy.com/raw.githubusercontent.com/wuyou69/WuYouLab/main/WuYouLab.sh && chmod +x WuYouLab.sh && sed -i 's/canshu=\"default\"/canshu=\"V6\"/g' ./WuYouLab.sh"
			else
				SH_Update_task="curl -sS -O https://raw.githubusercontent.com/wuyou69/WuYouLab/main/WuYouLab.sh && chmod +x WuYouLab.sh"
			fi
			check_crontab_installed
			(crontab -l | grep -v "WuYouLab.sh") | crontab -
